...
Mat SWT(const Mat &input, bool dark_on_light) {
    assert ( input.depth() == CV_8U );
    assert ( input.channels() == 3 );

    std::cout << "Running SWT with dark_on_light " << dark_on_light << std::endl;
    ...
    for( int row = 0; row < input.rows; row++ ){
        ...
        for ( int col = 0; col < input.cols; col++ ){
            *ptr++ = -1;  // 初始SWTImage每个像素值都为-1，作为初始化的标志（论文中设为无限大）
        }
    }
...
//    cvtColor (output4, output5, CV_GRAY2RGB);
    ...
    // strokeWidthTransform 需要的已知条件：边缘图，每个像素点上的梯度，输出 SWT 每个像素所属区域的笔画宽度
    ...
    for( int row = 0; row < edgeImage.rows; row++ ){
        ...
        for ( int col = 0; col < edgeImage.cols; col++ ){
            if (*ptr > 0) {
                ...
                // 计算改点梯度，沿着梯度爬行，直到找到在边缘上的点（边缘上的点为白色，即值不为0）
                ...
                while (true) {
                    ...
                    if ((int)(floor(curX)) != curPixX || (int)(floor(curY)) != curPixY) {
                        ...
                        if (edgeImage.at<uchar>(curPixY, curPixX) > 0) {
                            ...
                            // p,q 梯度方向偏差不超过直角，将p、q之间的所有点的SWT都设为p、q之间的距离
...
void SWTMedianFilter (Mat& SWTImage, std::vector<Ray> & rays) {
    for (auto& rit : rays) {
        ...
        // 对每个 ray 根据SWT排序，求得中值，并用这个中值 min(中值，原来值) 来更新 ray 上的所有像素
            ...
            if ( (compMedians[i]/compMedians[j] <= 2.0 || compMedians[j]/compMedians[i] <= 2.0) &&
                ...
                if (dist < 9*(float)(std::max(std::min(compDimensions[i].x,compDimensions[i].y),std::min(compDimensions[j].x,compDimensions[j].y)))
                    ...
                    */
                    ...
                    /*std::cout << c.p << " " << c.q << std::endl;
                    std::cout << c.direction.x << " " << c.direction.y << std::endl;
                    std::cout << compCenters[c.p].x << " " << compCenters[c.p].y << std::endl;
                    std::cout << compCenters[c.q].x << " " << compCenters[c.q].y << std::endl;
                    std::cout << std::endl;
                        ...
                        if (chains[i].p == chains[j].p) {
                            if (acos(chains[i].direction.x * -chains[j].direction.x + chains[i].direction.y * -chains[j].direction.y) < strictness) {
                                  /*      if (chains[i].p == chains[i].q || chains[j].p == chains[j].q) {
                                        ...
                                        std::cout << chains[i].p << " " << chains[i].q << std::endl;
                                        std::cout << chains[j].p << " " << chains[j].q << std::endl;
                                std::cout << compCenters[chains[i].q].x << " " << compCenters[chains[i].q].y << std::endl;
                                std::cout << compCenters[chains[i].p].x << " " << compCenters[chains[i].p].y << std::endl;
                                std::cout << compCenters[chains[j].q].x << " " << compCenters[chains[j].q].y << std::endl;
                                std::cout << std::endl; */
...
/*
                                ...
                                std::cout << chains[i].p << " " << chains[i].q << std::endl;
                                std::cout << chains[j].p << " " << chains[j].q << std::endl;
                                std::cout << chains[i].direction.x << " " << chains[i].direction.y << std::endl;
                                std::cout << chains[j].direction.x << " " << chains[j].direction.y << std::endl;
                                std::cout << compCenters[chains[i].q].x << " " << compCenters[chains[i].q].y << std::endl;
                                std::cout << compCenters[chains[i].p].x << " " << compCenters[chains[i].p].y << std::endl;
                                std::cout << compCenters[chains[j].p].x << " " << compCenters[chains[j].p].y << std::endl;
                                std::cout << std::endl; */
    ...
    std::vector<cv::Rect> detect_in_gray(Mat &gray_src) {
//        Mat src = SWT(input, dark_on_light);
        cv::cvtColor(gray_src, gray_src, cv::COLOR_GRAY2RGB);

        // Extract channels to be processed individually
        std::vector<Mat> channels;

        cv::text::computeNMChannels(gray_src, channels);
        ...
        erGrouping(gray_src, channels, regions, region_groups, groups_boxes, cv::text::ERGROUPING_ORIENTATION_HORIZ);
...
//        groups_boxes.erase(std::unique(groups_boxes.begin(), groups_boxes.end()), groups_boxes.end());
    ...
    std::vector<cv::Rect> detect(const Mat &src, bool dark_on_light) {
        auto gray_src = SWT(src, dark_on_light);
        return detect_in_gray(gray_src);
    }
