...
/**
 *  KEY FUNCTION  output gray
 * @param input   "dirty" pic
 * @param dark_on_light 0: text is ligther, 1: text is darker
 * @return "clean" pic after SWT
 */
cv::Mat SWT(const cv::Mat &input, bool dark_on_light);


/**
 *
 * @param gray_src gray pic
 * @param dark_on_light 0: text is lighter, 1: text is darker
 * @return Rects coresponding to each text area
 */
    std::vector<cv::Rect> detect_in_gray(cv::Mat &gray_src);

    std::vector<cv::Rect> detect(const cv::Mat &src, bool dark_on_light);
